...
impl Editor {
    ...
    pub(crate) fn refresh_inlay_hints(
        ...
        let Some((semantics_provider, project)) =
            self.semantics_provider().zip(self.project().clone())
        else {
            return;
        };
...
"#
        ...
        editor
            .update(cx, |editor, _window, cx| {
                ...
                assert_eq!(sorted_cached_hint_labels(editor, cx), expected_labels);
                assert_eq!(visible_hint_labels(editor, cx), expected_labels);
            })
            .unwrap();
